...
type CartContextType = { items: CartItem[]; addItem: (product: Product, size: string, breed: string) => Promise<void>; removeItem: (itemId: string) => Promise<void>; updateQuantity: (itemId: string, qty: number) => Promise<void>; clearCart: () => Promise<void>; total: number; count: number; isOpen: boolean; error: string; openCart: () => void; closeCart: () => void };
...
export function CartProvider({ children }: { children: ReactNode }) {
  ...
  const addItem = async (product: Product, size: string, breed: string) => { requireUser(); setError(""); const updated = await cartApi.add(product.id, breed, size); setItems(updated); setIsOpen(true); };
